...
class OptionsReader():
    """
    Class for reading options from the file
    """
    ...
    def __init__(self, opt_path) -> None:
        ...
        with open(opt_path,"r") as file:
            for line in file:
                line_raw = self.delete_comments(line).strip()
                if(len(line_raw) <= 0):
                    continue
                if(line_raw[0] == "-"):
    ...
    def iterate_options(self, reset = False)-> Algorithm_Option | None:
        """Function is an iterator, returns next option, pass argument False to reset the index from zero"""
        if(reset):
            self.iterate_idx = 0
    ...
    def change_option_by_idx(self, value: float|int, idx:int)-> None:
        """Function changes the value of function by index"""
    ...
    def change_option_by_name(self, value: float|int, name:str) -> None:
        for option in self.opts_array:
            if option.is_matching_name(name):
    ...
    def save_options(self, path_to_save:str) -> None:
        class_name = ""
